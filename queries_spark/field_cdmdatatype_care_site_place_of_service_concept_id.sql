...
-- cdm_table_name: CARE_SITE
-- cdm_field_name: PLACE_OF_SERVICE_CONCEPT_ID
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_cdmdatatype_care_site_place_of_service_concept_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('CARE_SITE' AS STRING) as cdm_table_name,
  CAST('PLACE_OF_SERVICE_CONCEPT_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 cdmTable.* 
 FROM {cdm_schema}.CARE_SITE cdmTable
 WHERE CASE WHEN CAST(abs(cdmTable.PLACE_OF_SERVICE_CONCEPT_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
