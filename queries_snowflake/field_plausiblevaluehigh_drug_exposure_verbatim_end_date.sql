...
-- checkId: field_plausiblevaluehigh_drug_exposure_verbatim_end_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('VERBATIM_END_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DRUG_EXPOSURE cdmTable
      	WHERE cast(cdmTable.VERBATIM_END_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
