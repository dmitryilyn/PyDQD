...
-- cdm_field_name: EPISODE_START_DATETIME
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_episode_episode_start_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('EPISODE_START_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.EPISODE cdmTable
      	WHERE cast(cdmTable.EPISODE_START_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
