...
-- checkId: field_plausiblevaluehigh_drug_exposure_drug_exposure_start_date
-- thresholdValue: 1
SELECT 
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DRUG_EXPOSURE cdmTable
      	WHERE cast(cdmTable.DRUG_EXPOSURE_START_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
