...
-- checkId: field_cdmdatatype_payer_plan_period_payer_source_concept_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('PAYER_SOURCE_CONCEPT_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 WHERE CASE WHEN CAST(abs(cdmTable.PAYER_SOURCE_CONCEPT_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
