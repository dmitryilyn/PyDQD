...
-- cdm_field_name: CONDITION_START_DATETIME
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_condition_occurrence_condition_start_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('CONDITION_START_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.CONDITION_OCCURRENCE cdmTable
      	WHERE cast(cdmTable.CONDITION_START_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
