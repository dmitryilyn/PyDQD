...
-- cdm_field_name: DRUG_ERA_START_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_drug_era_drug_era_start_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('DRUG_ERA_START_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DRUG_ERA cdmTable
      	WHERE cast(cdmTable.DRUG_ERA_START_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
