...
SELECT 
  ...
  ,'The number and percent of records that have a value in the PAYER_PLAN_PERIOD_ID field in the COST table that does not exist in the PAYER_PLAN_PERIOD table.' as check_description
  ...
  ,'' as warning
  ,'' as error
  ,'field_isforeignkey_cost_payer_plan_period_id' as checkid
  ,0 as is_error
  ,0 as not_applicable
...
FROM (
	...
	FROM (
		...
		FROM {cdm_schema}.COST cdmTable
		  LEFT JOIN {cdm_schema}.PAYER_PLAN_PERIOD fkTable
