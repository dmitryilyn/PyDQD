...
-- cdm_field_name: MEASUREMENT_DATETIME
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_measurement_measurement_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('MEASUREMENT_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.MEASUREMENT cdmTable
      	WHERE cast(cdmTable.MEASUREMENT_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
