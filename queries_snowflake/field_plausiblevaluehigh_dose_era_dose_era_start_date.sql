...
-- cdm_field_name: DOSE_ERA_START_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_dose_era_dose_era_start_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('DOSE_ERA_START_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DOSE_ERA cdmTable
      	WHERE cast(cdmTable.DOSE_ERA_START_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
