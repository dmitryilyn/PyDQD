...
SELECT 
  ...
  ,'A yes or no value indicating if the DISCHARGE_TO_CONCEPT_ID in the VISIT_OCCURRENCE is the expected data type based on the specification. Only checks integer fields.' as check_description
  ...
  ,'' as warning
  ,'' as error
  ,'field_cdmdatatype_visit_occurrence_discharge_to_concept_id' as checkid
  ,0 as is_error
  ,0 as not_applicable
...
(
	...
	(
		...
		WHERE 
			(IS_REAL(TRY_TO_NUMERIC(cdmTable.DISCHARGE_TO_CONCEPT_ID)) = 0 
			OR (IS_REAL(TRY_TO_NUMERIC(cdmTable.DISCHARGE_TO_CONCEPT_ID)) = 1 
