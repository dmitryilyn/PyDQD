...
-- cdm_field_name: DRUG_EXPOSURE_END_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_drug_exposure_drug_exposure_end_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('DRUG_EXPOSURE_END_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DRUG_EXPOSURE cdmTable
      	WHERE cast(cdmTable.DRUG_EXPOSURE_END_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
