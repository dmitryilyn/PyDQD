...
-- cdm_field_name: DEVICE_EXPOSURE_START_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_device_exposure_device_exposure_start_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('DEVICE_EXPOSURE_START_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DEVICE_EXPOSURE cdmTable
      	WHERE cast(cdmTable.DEVICE_EXPOSURE_START_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
