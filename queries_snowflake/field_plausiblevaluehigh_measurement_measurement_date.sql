...
-- checkId: field_plausiblevaluehigh_measurement_measurement_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('MEASUREMENT_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.MEASUREMENT cdmTable
      	WHERE cast(cdmTable.MEASUREMENT_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
