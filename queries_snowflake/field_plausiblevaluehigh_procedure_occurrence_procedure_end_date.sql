...
-- cdm_field_name: PROCEDURE_END_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_procedure_occurrence_procedure_end_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('PROCEDURE_END_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.PROCEDURE_OCCURRENCE cdmTable
      	WHERE cast(cdmTable.PROCEDURE_END_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
