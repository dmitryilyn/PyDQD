...
-- cdm_field_name: VISIT_SOURCE_CONCEPT_ID
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_cdmdatatype_visit_occurrence_visit_source_concept_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('VISIT_SOURCE_CONCEPT_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 WHERE CASE WHEN CAST(abs(cdmTable.VISIT_SOURCE_CONCEPT_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
