...
SELECT 
  ...
  ,'The number and percent of records that have a value in the VISIT_DETAIL_PARENT_ID field in the VISIT_DETAIL table that does not exist in the VISIT_DETAIL table.' as check_description
  ,'VISIT_DETAIL' as cdm_table_name
  ,'VISIT_DETAIL_PARENT_ID' as cdm_field_name
  ,'NA' as concept_id
  ...
  ,'' as warning
  ,'' as error
  ,'field_isforeignkey_visit_detail_visit_detail_parent_id' as checkid
  ,0 as is_error
  ,0 as not_applicable
...
FROM (
    ...
    FROM (
        ...
        FROM {cdm_schema}.VISIT_DETAIL cdmTable
            LEFT JOIN 
                {cdm_schema}.VISIT_DETAIL fkTable 
