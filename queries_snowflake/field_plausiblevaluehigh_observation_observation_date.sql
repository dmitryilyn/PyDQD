...
-- cdm_field_name: OBSERVATION_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_observation_observation_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('OBSERVATION_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.OBSERVATION cdmTable
      	WHERE cast(cdmTable.OBSERVATION_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
