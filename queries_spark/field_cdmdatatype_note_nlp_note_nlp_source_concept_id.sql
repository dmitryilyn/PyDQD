...
-- cdm_field_name: NOTE_NLP_SOURCE_CONCEPT_ID
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_cdmdatatype_note_nlp_note_nlp_source_concept_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('NOTE_NLP' AS STRING) as cdm_table_name,
  CAST('NOTE_NLP_SOURCE_CONCEPT_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 WHERE CASE WHEN CAST(abs(cdmTable.NOTE_NLP_SOURCE_CONCEPT_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
