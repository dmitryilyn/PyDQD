...
-- cdm_field_name: VISIT_DETAIL_END_DATETIME
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_visit_detail_visit_detail_end_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('VISIT_DETAIL_END_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.VISIT_DETAIL cdmTable
      	WHERE cast(cdmTable.VISIT_DETAIL_END_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
