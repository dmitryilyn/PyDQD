...
SELECT 
  ...
  ,'The number and percent of records with a value in the CDM_RELEASE_DATE field of the CDM_SOURCE that occurs after the date in the SOURCE_RELEASE_DATE.' as check_description
  ...
  ,'' as warning
  ,'' as error
  ,'field_plausiblestartbeforeend_cdm_source_cdm_release_date' as checkid
  ,0 as is_error
  ,0 as not_applicable
...
CDM_RELEASE_DATE is the start date and SOURCE_RELEASE_DATE is the end date.
...
(
    ...
    (
        ...
        WHERE cdmTable.CDM_RELEASE_DATE IS NOT NULL 
            AND cdmTable.SOURCE_RELEASE_DATE IS NOT NULL 
            AND CAST(cdmTable.CDM_RELEASE_DATE AS DATE) > CAST(cdmTable.SOURCE_RELEASE_DATE AS DATE)
        /*violatedRowsEnd*/
    ) violated_rows
) violated_row_count,
(
