...
-- checkId: field_cdmdatatype_payer_plan_period_payer_plan_period_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('PAYER_PLAN_PERIOD_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 WHERE CASE WHEN CAST(abs(cdmTable.PAYER_PLAN_PERIOD_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
