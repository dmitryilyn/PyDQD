...
-- checkId: field_cdmdatatype_device_exposure_unit_concept_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('UNIT_CONCEPT_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 WHERE CASE WHEN CAST(abs(cdmTable.UNIT_CONCEPT_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
