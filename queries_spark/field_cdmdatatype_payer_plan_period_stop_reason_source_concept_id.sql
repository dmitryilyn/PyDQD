...
-- cdm_field_name: STOP_REASON_SOURCE_CONCEPT_ID
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_cdmdatatype_payer_plan_period_stop_reason_source_concept_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('STOP_REASON_SOURCE_CONCEPT_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 WHERE CASE WHEN CAST(abs(cdmTable.STOP_REASON_SOURCE_CONCEPT_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
