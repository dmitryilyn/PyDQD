...
-- cdm_field_name: OBSERVATION_PERIOD_END_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_observation_period_observation_period_end_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('OBSERVATION_PERIOD' AS STRING) as cdm_table_name,
  CAST('OBSERVATION_PERIOD_END_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.OBSERVATION_PERIOD cdmTable
      	WHERE cast(cdmTable.OBSERVATION_PERIOD_END_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
