...
-- cdm_field_name: COHORT_START_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_cohort_cohort_start_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('COHORT' AS STRING) as cdm_table_name,
  CAST('COHORT_START_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
		...
		cdmTable.*
    	FROM {cdm_schema}.COHORT cdmTable
      	WHERE cast(cdmTable.COHORT_START_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
