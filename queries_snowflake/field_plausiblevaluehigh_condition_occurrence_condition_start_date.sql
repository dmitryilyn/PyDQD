...
-- checkId: field_plausiblevaluehigh_condition_occurrence_condition_start_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('CONDITION_START_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.CONDITION_OCCURRENCE cdmTable
      	WHERE cast(cdmTable.CONDITION_START_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
