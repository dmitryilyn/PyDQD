...
-- checkId: field_plausiblevaluehigh_note_nlp_nlp_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('NLP_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.NOTE_NLP cdmTable
      	WHERE cast(cdmTable.NLP_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
