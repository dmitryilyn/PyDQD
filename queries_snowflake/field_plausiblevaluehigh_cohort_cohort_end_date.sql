...
-- cdm_field_name: COHORT_END_DATE
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_cohort_cohort_end_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('COHORT_END_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.COHORT cdmTable
      	WHERE cast(cdmTable.COHORT_END_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
