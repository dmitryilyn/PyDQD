...
-- cdm_table_name: EPISODE_EVENT
-- cdm_field_name: EPISODE_EVENT_FIELD_CONCEPT_ID
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_cdmdatatype_episode_event_episode_event_field_concept_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('EPISODE_EVENT' AS STRING) as cdm_table_name,
  CAST('EPISODE_EVENT_FIELD_CONCEPT_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 cdmTable.* 
 FROM {cdm_schema}.EPISODE_EVENT cdmTable
 WHERE CASE WHEN CAST(abs(cdmTable.EPISODE_EVENT_FIELD_CONCEPT_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
 AND cdmTable.EPISODE_EVENT_FIELD_CONCEPT_ID IS NOT NULL
 /*violatedRowsEnd*/
 ) violated_rows
) violated_row_count cross join (SELECT 
 COUNT(*) AS num_rows
 FROM {cdm_schema}.EPISODE_EVENT
) denominator
) cte
