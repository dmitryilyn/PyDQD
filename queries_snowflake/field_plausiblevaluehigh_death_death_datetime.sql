...
-- checkId: field_plausiblevaluehigh_death_death_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('DEATH_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DEATH cdmTable
      	WHERE cast(cdmTable.DEATH_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
