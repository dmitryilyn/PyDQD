...
-- checkId: field_plausiblevaluehigh_condition_occurrence_condition_end_datetime
-- thresholdValue: 1
SELECT 
...
(
	...
	(
    	...
    	FROM {cdm_schema}.CONDITION_OCCURRENCE cdmTable
      	WHERE cast(cdmTable.CONDITION_END_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
