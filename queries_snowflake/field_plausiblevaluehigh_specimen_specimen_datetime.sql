...
-- cdm_field_name: SPECIMEN_DATETIME
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_specimen_specimen_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('SPECIMEN_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.SPECIMEN cdmTable
      	WHERE cast(cdmTable.SPECIMEN_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
