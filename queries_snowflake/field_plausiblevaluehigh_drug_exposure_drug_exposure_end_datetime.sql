...
-- cdm_field_name: DRUG_EXPOSURE_END_DATETIME
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_drug_exposure_drug_exposure_end_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('DRUG_EXPOSURE_END_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DRUG_EXPOSURE cdmTable
      	WHERE cast(cdmTable.DRUG_EXPOSURE_END_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
