...
-- cdm_field_name: OBSERVATION_DATETIME
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_plausiblevaluehigh_observation_observation_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('OBSERVATION_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.OBSERVATION cdmTable
      	WHERE cast(cdmTable.OBSERVATION_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
