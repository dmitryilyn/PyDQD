...
-- checkId: field_plausiblevaluehigh_visit_occurrence_visit_end_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('VISIT_END_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.VISIT_OCCURRENCE cdmTable
      	WHERE cast(cdmTable.VISIT_END_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
