...
-- checkId: field_plausiblevaluehigh_device_exposure_device_exposure_start_datetime
-- thresholdValue: 1
SELECT 
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DEVICE_EXPOSURE cdmTable
      	WHERE cast(cdmTable.DEVICE_EXPOSURE_START_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
