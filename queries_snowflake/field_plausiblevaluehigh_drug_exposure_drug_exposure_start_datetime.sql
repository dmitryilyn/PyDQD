...
-- checkId: field_plausiblevaluehigh_drug_exposure_drug_exposure_start_datetime
-- thresholdValue: 1
SELECT 
  ...
  CAST('DRUG_EXPOSURE_START_DATETIME' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.DRUG_EXPOSURE cdmTable
      	WHERE cast(cdmTable.DRUG_EXPOSURE_START_DATETIME as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
