...
-- cdm_field_name: DRUG_EXPOSURE_COUNT
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_cdmdatatype_drug_era_drug_exposure_count
-- thresholdValue: 0
SELECT 
  ...
  CAST('DRUG_ERA' AS STRING) as cdm_table_name,
  CAST('DRUG_EXPOSURE_COUNT' AS STRING) as cdm_field_name,
...
FROM (
 ...
 WHERE CASE WHEN CAST(abs(cdmTable.DRUG_EXPOSURE_COUNT) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
