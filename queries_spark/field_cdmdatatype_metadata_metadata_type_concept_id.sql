...
-- cdm_field_name: METADATA_TYPE_CONCEPT_ID
-- concept_id: NA
-- unit_concept_id: NA
...
-- checkId: field_cdmdatatype_metadata_metadata_type_concept_id
-- thresholdValue: 0
SELECT 
  ...
  CAST('METADATA' AS STRING) as cdm_table_name,
  CAST('METADATA_TYPE_CONCEPT_ID' AS STRING) as cdm_field_name,
...
FROM (
 ...
 WHERE CASE WHEN CAST(abs(cdmTable.METADATA_TYPE_CONCEPT_ID) AS DOUBLE) IS NOT NULL THEN 1 ELSE 0 END = 0 
