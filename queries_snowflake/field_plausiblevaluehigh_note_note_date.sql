...
-- checkId: field_plausiblevaluehigh_note_note_date
-- thresholdValue: 1
SELECT 
  ...
  CAST('NOTE_DATE' AS STRING) as cdm_field_name,
...
(
	...
	(
    	...
    	FROM {cdm_schema}.NOTE cdmTable
      	WHERE cast(cdmTable.NOTE_DATE as date) > cast(DATEADD(dd,1,CURRENT_DATE) as date)
		/*violatedRowsEnd*/
	) violated_rows
) violated_row_count,
(
